...
def test_from_env_missing_database_url_raises_clear_error(monkeypatch):
    # 隔离 .env 文件影响：mock load_dotenv 防止本地 .env 中的 DATABASE_URL 干扰测试
    # 否则 monkeypatch.delenv 后，load_dotenv(override=False) 会重新从 .env 加载，导致测试失败
    monkeypatch.setattr("email_agent.config.settings.load_dotenv", lambda override=False: None)
